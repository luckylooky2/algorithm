...
console.log(map.slice(1002 - 10, 1337 + 10 + 1).join(" "));
...
// 이유는? 어떤 방향으로 먼저 처리할 것인가가 중요하기 때문인 것 같다
// k -> n / [-1, 1] : 325 326 327 328 329 330 331 332 333 334 "336" 335 334 333 332 331 330
// k -> n / [1, -1] : 325 326 327 328 329 330 331 332 333 334 "335" 335 334 333 332 331 330
// n -> k / [-1, 1] : 325 326 327 328 329 330 331 332 333 334 "335" 335 334 333 332 331 330
// priority queue : 올라가는 방향이면 작은 인덱스(-1)를 먼저 처리하고, 내려가는 방향이면 큰 인덱스(+1)를 먼저 처리한다
// deque : 2를 곱해주는 것은 push_front, -+1은 push_back으로 처리한다(어떤 인덱스를 먼저 처리해도 상관 없음)
